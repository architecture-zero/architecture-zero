...
from app.history import (
    ...
    upsert_session_meta, get_session_meta,   # chat reads both; sessions router imports its own
)
from app.auth import AuthMiddleware
from app.users import owner_exists   # the boot banner's occupancy check; the rest moved
from app.jwt_auth import get_current_user
from app.config import init_config_db, get_config, get_system_prompt
...
# The startup hooks drive these three; routers/kb.py imports its own from the
# same module. One-way: main -> ingest_sync.
from app.ingest_sync import _sync_knowledge_dir, _sync_docs, _watch_knowledge_dir
# The seed loader runs at startup; the rest of the engine belongs to the evals
# router. runtime_config is imported as a MODULE so _startup_ingest_active is
# written through it rather than snapshotted.
...
from app.security import (check_rate_limit, check_injection, client_ip_from_request,
                          setup_claim_code, claim_code_source)
from app.audit import log_audit_entry, purge_old_entries
from app.metrics import increment, record_request
from app.peers import get_peers, query_peer_kb   # the rest moved to routers/peers.py
# Shared with the routers. Direction is main -> routers -> runtime_config, and
# nothing under app/ may import app.main.
from app.runtime_config import (_config_or_default, DEFAULT_MODEL,
                                RAG_ONLY_MODE, RAG_SIMILARITY_THRESHOLD,
                                ALLOW_GUEST_MODE, MAX_CONTEXT_TOKENS,
                                _BLOCKLIST, _SAFETY_RULES, _NON_OWNER_RULES,
